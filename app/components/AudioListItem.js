...
const renderPlayPauseIcon = isPlaying => {
    return isPlaying ? <Entypo name='controller-paus' size={24} color={color.ACTIVE_FONT} /> : <Entypo name='controller-play' size={24} color={color.ACTIVE_FONT} />  // play icon
}
...
const AudioListItem = ({title = '', duration = 0, onOptionPress, onAudioPress, isPlaying, activeListItem}) => {
    return (
        <>
        <View style={styles.container}>
            <TouchableWithoutFeedback onPress={onAudioPress}>
                <View style={styles.leftContainer}>
                    <View style={{...styles.thumbnail, backgroundColor: renderColorThumbnail(activeListItem) }}>
